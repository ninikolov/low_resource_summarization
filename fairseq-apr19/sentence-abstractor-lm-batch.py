...
def main(args):
    ...
    with tqdm() as pbar:
        for inputs in buffered_read(args.input, args.buffer_size):
            ...
            abstractive_summaries = [[] for j in range(args.buffer_size)]
            final_clean_summaries = [[] for j in range(args.buffer_size)]
            ...
            for sentence_num in range(max_input_sentence_length):
                ...
                # Get all sentences from batch that are at the current position
                sentence_position = []
                for input_text in buffer_inputs:
                    if len(input_text) > sentence_num:
                        sentence_position.append(input_text[sentence_num])
                    else:
                        sentence_position.append("")
                ...
                current_history = ["<s> " + " <s> ".join(summary) if len(summary) > 0 else "<s>"
                ...
                # if sentence_num != 0: # Disable for first sentence
                #     # Sort hypotheses according to LM predictions
                #     for sent in range(len(translations)):
                #         translations[sent] = sorted(translations[sent],
                #                                     key=lambda r: r['lm_score'], reverse=True)
                ...
                for i, (id, src_tokens, hypos) in enumerate(sorted(results, key=lambda x: x[0])):
                    ...
                    for hypo in hypos[:min(len(hypos), args.nbest)]:
                        ...
                        abstractive_summaries[i].append(hypo_str)
                        # Remove BPE for final clean summary
                        final_clean_summaries[i].append(process_bpe_symbol(hypo_str, "@@ "))

            for input_text, clean_summary in zip(buffer_inputs, final_clean_summaries):
                pbar.update()
                # Remove any padded sentences
                clean_summary = clean_summary[:len(input_text)]
                ...
                if not args.no_print:
                    ...
                    print()
                    print("Summary:")
                    for i, s in enumerate(clean_summary):
                        print("  {}) {}".format(i, s))
                    print("*"*50)

                if args.detokenize:
                    clean_summary = [detokenizer.detokenize(s, return_str=True) for s in clean_summary]
                    out_file.write("{}\n".format(" ".join(clean_summary)))
                else:
                    out_file.write("{}\n".format(" <s> ".join(clean_summary)))
...
def cli_main():
    ...
    parser.add_argument('--detokenize', help="detokenize output summary sentences with NLTK", default=False, type=bool)
